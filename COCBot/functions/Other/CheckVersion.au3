...
Func CheckVersion()
	...
	If $Temp <> "" And Not @error Then
		...
		If _VersionCompare($g_iBotVersionN, $g_sBotGitVersion) = -1 Then
			SetLog("WARNING, YOUR VERSION (" & $g_iBotVersionN & ") IS OUT OF DATE.", $COLOR_SUCCESS, "Candara", 9)
		...
		ElseIf _VersionCompare($g_iBotVersionN, $g_sBotGitVersion) = 0 Then
			SetLog(" WELCOME CHIEF, YOU HAVE THE LATEST LiGHT MOD VERSION", $COLOR_SUCCESS, "Candara", 9)
		Else
			SetLog("YOU ARE USING A FUTURE VERSION CHIEF!", $COLOR_SUCCESS, "Candara", 9)
		EndIf
		SetLog("MODIFIED BY  ELOY." & "  COMMITTED TO THE OFFICIAL VERSION.", $COLOR_SUCCESS, "Candara", 9)
		SetLog(" ")
	Else
		SetDebugLog($Temp)
	EndIf
